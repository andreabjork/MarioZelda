...
Character.prototype.relX = 200;
Character.prototype.relY = 483;
...
Character.prototype.update = function (du) {
    ...
    if (this.velX < 0) {
        if (viewPort.cx >= 0) {
            if (this.relX > 25 && !blocks.left) {
                this.relX += this.velX*du;
            } 
        } else {
            if (this.relX > 200 && !blocks.left) {
                this.relX += this.velX*du;
            }
        }
    }  else if (this.velX > 0) {
        if (viewPort.cx <= entityManager._level[0].Blocks[13].length*(-X)+g_canvas.width) {
            if (this.relX < g_canvas.width-25 && !blocks.right) {
                this.relX += this.velX*du;
            }
        } else if (this.relX < 400 && !blocks.right) {
            this.relX += this.velX*du;
        }
    }
    var nextY = this.relY + this.velY*du
    if (nextY < 100) {
        this.relY = 100;
